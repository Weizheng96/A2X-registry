...
> **0.1.6 起依赖布局变更**：默认 `pip install` 仅装 SDK 必需的轻量依赖（Agent Team 场景）；A2X 分类构建、向量检索、评估 CLI 等增强功能放到 `[full]` extras 里。本 README 介绍**全量模式**（含搜索/构建/评估），需要附加 `[full]`。Agent Team 精简部署见 [README_forAgentTeam.md](README_forAgentTeam.md)。

从 GitHub 标签安装（推荐，不需要源码）：

```bash
pip install "a2x-registry[full] @ git+https://github.com/Weizheng96/A2X-registry.git@v0.1.6"
```
...
| extras | 包含 | 启用功能 |
|---|---|---|
| `[vector]` | numpy / sentence-transformers / chromadb | A2X 分类构建、向量检索、传统全量检索 |
| `[evaluation]` | tqdm | `a2x-evaluate-*` 评估 CLI 的进度条 |
| `[full]` | 上述全部 | 等价旧版（≤ 0.1.5）默认安装 |

> 旧版（≤ 0.1.5）用户升级后，记得加 `[full]`：`pip install -U 'a2x-registry[full]' …`
