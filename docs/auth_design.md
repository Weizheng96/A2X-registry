...
- [reference/分析报告/auth_design.md](../reference/分析报告/auth_design.md) — 威胁模型、Phase 2/3 留口、行业实践对照（PyPI / GitHub PAT / HF / Jenkins）
- [reference/分析报告/auth_design_sequence.md](../reference/分析报告/auth_design_sequence.md) — admin / provider / user 三视角的时序图，覆盖 token 生成 → 分发 → 保存全链路
