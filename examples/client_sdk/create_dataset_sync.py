...
1. default formats (SDK injects ``{"a2a": "v0.0"}``)
2. explicit ``formats=None`` (omit the field, let backend choose defaults)
...
def main() -> None:
    ...
    with A2XClient(base_url=base_url) as client:
        ...
        try:
            client.create_dataset("example_sync_bad_formats", formats={})
        ...
        first = client.create_dataset(ds_duplicate)
        print(f"  first create ok: {first.dataset}")
        try:
            client.create_dataset(ds_duplicate)
        except ValidationError as exc:
            print(f"  caught: {type(exc).__name__}")
            print(f"  status_code: {exc.status_code}")
            print(f"  payload: {exc.payload}")
