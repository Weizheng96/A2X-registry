...
Run:
    python examples/client_sdk/update_agent_async.py
...
async def main() -> None:
    ...
    ownership_file = Path(tempfile.gettempdir()) / "a2x_example_update_async_owned.json"
    ...
    async with AsyncA2XClient(base_url=base_url, ownership_file=ownership_file) as client:
        ...
        reg = await client.register_agent(
            ds,
            make_card("Updater Async", "原始描述"),
            service_id="agent_update_async",
            persistent=True,
        )
        print("\n[setup]")
        ...
        print("\n[non-taxonomy update]")
        patch2 = await client.update_agent(
            ds,
            reg.service_id,
        ...
        doomed = await client.register_agent(
            ds,
            make_card("To Be Deleted Async", "soon gone"),
            service_id="agent_update_async_deleted",
            persistent=True,
        )
