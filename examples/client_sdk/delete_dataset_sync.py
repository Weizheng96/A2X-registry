...
Run:
    python examples/client_sdk/delete_dataset_sync.py
...
def main() -> None:
    ...
    with A2XClient(base_url=base_url) as client:
        ...
        ensure_absent(client, ds_success)
        ensure_absent(client, ds_missing)

        # 1) Successful delete: create first, then delete.
        client.create_dataset(ds_success)
        result = client.delete_dataset(ds_success)
        print_result("successful delete", result)

        # 2) ValidationError: dataset does not exist.
        print("\n[validation error: dataset missing]")
        try:
            client.delete_dataset(ds_missing)
